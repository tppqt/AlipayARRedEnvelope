//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "APPassBaseDetailViewController.h"

#import "APSKLaunchpadDelegate.h"
#import "UIActionSheetDelegate.h"

@class APActionSheet, APExceptionView, APPassDetailModel, APSKLaunchpad, KABAOPRODPassShareInfo, NSString;

@interface APPassDetailViewController : APPassBaseDetailViewController <APSKLaunchpadDelegate, UIActionSheetDelegate>
{
    _Bool _isQueryLocationing;
    APPassDetailModel *_detailModel;
    KABAOPRODPassShareInfo *_shareInfo;
    APSKLaunchpad *_pad;
    APActionSheet *_popoverActionSheet;
    APActionSheet *_deleteActionSheet;
    APExceptionView *_emptyView;
    double _brightness;
    double _maxBright;
}

@property(nonatomic) double maxBright; // @synthesize maxBright=_maxBright;
@property(nonatomic) double brightness; // @synthesize brightness=_brightness;
@property(retain, nonatomic) APExceptionView *emptyView; // @synthesize emptyView=_emptyView;
@property(nonatomic) _Bool isQueryLocationing; // @synthesize isQueryLocationing=_isQueryLocationing;
@property(retain, nonatomic) APActionSheet *deleteActionSheet; // @synthesize deleteActionSheet=_deleteActionSheet;
@property(retain, nonatomic) APActionSheet *popoverActionSheet; // @synthesize popoverActionSheet=_popoverActionSheet;
@property(retain, nonatomic) APSKLaunchpad *pad; // @synthesize pad=_pad;
@property(retain, nonatomic) KABAOPRODPassShareInfo *shareInfo; // @synthesize shareInfo=_shareInfo;
@property(retain, nonatomic) APPassDetailModel *detailModel; // @synthesize detailModel=_detailModel;
- (void).cxx_destruct;
- (void)hideErrorView;
- (void)showError:(id)arg1 withModel:(id)arg2;
- (void)showModel:(id)arg1;
- (void)showEmpty:(id)arg1;
- (void)showLoading:(id)arg1;
- (void)queryDetailRPC;
- (void)load;
- (void)reload;
- (void)resetDetailModel;
- (void)queryLocation;
- (void)loadCache;
- (void)presentPassFailResult:(id)arg1;
- (void)presentPassSuccess:(id)arg1;
- (void)sharingLaunchpad:(id)arg1 didSelectChannel:(id)arg2;
- (void)sharingLaunchpadDidCancel:(id)arg1;
- (void)showSharePanel;
- (void)presentBtnPressed:(id)arg1;
- (void)clickDelete:(id)arg1;
- (void)clickShare:(id)arg1;
- (void)clickMore:(id)arg1;
- (void)back;
- (void)actionSheet:(id)arg1 didDismissWithButtonIndex:(long long)arg2;
- (_Bool)isCanUseAndSuport:(id)arg1;
- (void)reloadDataBarButtonItem;
- (void)restoreBright;
- (void)increaseBright;
- (void)removeBrightNoti;
- (void)addBrightNoti;
- (_Bool)needHighBright;
- (void)initializeBright;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (id)pageParams;
- (id)pageSpm;
- (void)viewDidLoad;
- (id)initWithBizType:(id)arg1 passId:(id)arg2 partnerId:(id)arg3 serialNumber:(id)arg4;
- (id)initWithBizType:(id)arg1 passInfo:(id)arg2;
- (id)init;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end


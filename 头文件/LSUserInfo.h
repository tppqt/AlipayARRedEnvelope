//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "NSCopying.h"

@class APContactInfo, NSDictionary, NSString;

@interface LSUserInfo : NSObject <NSCopying>
{
    _Bool _isGroupHost;
    int _userStatus;
    APContactInfo *_contactInfo;
    long long _userRole;
    NSString *_liveId;
    NSDictionary *_userExt;
}

@property(retain, nonatomic) NSDictionary *userExt; // @synthesize userExt=_userExt;
@property(nonatomic) int userStatus; // @synthesize userStatus=_userStatus;
@property(retain, nonatomic) NSString *liveId; // @synthesize liveId=_liveId;
@property(nonatomic) _Bool isGroupHost; // @synthesize isGroupHost=_isGroupHost;
@property(nonatomic) long long userRole; // @synthesize userRole=_userRole;
@property(retain, nonatomic) APContactInfo *contactInfo; // @synthesize contactInfo=_contactInfo;
- (void).cxx_destruct;
- (id)copyWithZone:(struct _NSZone *)arg1;

@end

